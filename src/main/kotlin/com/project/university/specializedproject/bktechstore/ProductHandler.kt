...
class ProductHandler(private val productRepository: ProductRepository) {
    ...
    suspend fun create(request: ServerRequest) : ServerResponse = ServerResponse.ok().bodyValueAndAwait("Create endpoints")
    suspend fun remove(request: ServerRequest) : ServerResponse = ServerResponse.ok().bodyValueAndAwait("Delete endpoints")
